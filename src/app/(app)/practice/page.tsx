...
export default function PracticePage() {
  ...
  if (isScenarioLoading || isCuesLoading) {
    return (
      <div className="max-w-5xl mx-auto h-[60vh] flex flex-col justify-center gap-6 p-4">
        <div className="h-12 w-1/3 bg-muted rounded-2xl animate-pulse" />
        <div className="h-110 w-full bg-muted rounded-3xl animate-pulse" />
      </div>
    );
  }
  ...
  const ReadingPaneToolbar = () => (
    <div className="flex items-center gap-1 px-3 py-2 border-b-2 border-border bg-muted/30 shrink-0 text-muted-foreground">
      ...
      <div className="ml-auto flex items-center gap-1">
        ...
        <Badge variant="secondary" className="font-bold text-[10px] uppercase bg-background shadow-sm border-2">
  ...
  const MailClient = () => {
    ...
    return (
    <Card className={`border-2 shadow-sm flex flex-col p-0 overflow-hidden ${compact ? 'h-auto max-h-[70vh]' : 'h-[62vh] max-h-160'} transition-all duration-500`}>
      <ReadingPaneToolbar />
      ...
      {/* Sender identity row */}
      <div className={`px-5 md:px-6 flex items-center gap-3 border-b-2 border-border shrink-0 ${compact ? 'pb-3' : 'pb-4'}`}>
      ...
      <CardContent className="flex-1 min-h-0 px-5 md:px-6 py-5 text-sm leading-relaxed overflow-y-auto whitespace-pre-wrap bg-background text-foreground/90">
        ...
        {scenario.links.length > 0 && (
          <div className="mt-6 space-y-2 border-t-2 border-dashed border-border pt-4">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Links in this message</p>
            {scenario.links.map((link, idx) => (
              <div key={idx} className="bg-muted/30 border-2 border-border p-2 rounded-lg text-primary text-xs break-all relative group cursor-help transition-colors hover:bg-muted/50">
      ...
      {scenario.attachmentName && (
        <div className="px-5 md:px-6 py-3 border-t-2 border-border bg-muted/20 shrink-0">
          <div className="inline-flex items-center gap-2 text-sm font-semibold bg-background border-2 border-border px-3 py-2 rounded-lg text-foreground max-w-full">
  ...
  const InboxList = () => (
    <Card className="border-2 shadow-sm p-0 overflow-hidden flex flex-col h-[62vh] max-h-160">
      <div className="flex items-center gap-2 px-4 py-3 border-b-2 border-border bg-muted/30 shrink-0">
      ...
      <div className="px-3 py-2 border-b-2 border-border shrink-0">
      ...
      <div className="flex-1 overflow-y-auto divide-y-2 divide-border">
        {/* Active (current scenario) row */}
        <div className="px-4 py-3 bg-primary/10 border-l-4 border-primary cursor-default">
          <div className="flex items-center justify-between gap-2 mb-1">
            <span className="font-bold text-sm text-foreground truncate">{senderName}</span>
            <span className="text-[11px] font-bold text-primary shrink-0">{timestamp}</span>
          </div>
          <p className="font-bold text-sm text-foreground truncate">{scenario.subject}</p>
  ...
  return (
    <div className="max-w-5xl mx-auto h-full flex flex-col relative pb-12 animate-in fade-in duration-500">
      ...
      <Dialog open={step === 'feedback'} onOpenChange={() => {}}>
        <DialogContent className="max-w-md p-0 border-2 overflow-hidden rounded-3xl gap-0 flex flex-col max-h-[90vh] [&>button]:hidden">
          {result && (
            <>
