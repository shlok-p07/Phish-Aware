...
import { Shield, Home, BookOpen, Target, User, Trophy, LogOut, PanelLeftClose, PanelLeftOpen, Settings } from "lucide-react";
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh flex flex-col md:flex-row bg-background">
      {/* Desktop Sidebar */}
      <aside className={`hidden md:flex ${collapsed ? "w-20" : "w-72"} flex-col border-r-2 border-border bg-card px-4 py-6 sticky top-0 h-dvh overflow-hidden transition-[width] duration-300 ease-in-out`}>
        ...
        {collapsed && (
          <button onClick={toggleCollapsed} className="flex items-center justify-center p-3 mb-2 rounded-2xl text-muted-foreground hover:bg-muted hover:text-foreground transition-colors hover:cursor-pointer" aria-label="Expand sidebar">
        ...
        <Link
          ...
          className={`flex items-center mb-8 rounded-2xl transition-all ${collapsed ? "justify-center p-2" : "gap-3 px-3 py-3"} ${pathname === "/profile" ? "bg-primary/10" : "bg-muted/50 hover:bg-muted"}`}
        >
        ...
        <nav className="flex-1 space-y-2">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} title={collapsed ? item.label : undefined} className={`flex items-center py-3.5 rounded-2xl transition-all font-semibold ${collapsed ? "justify-center px-0" : "gap-4 px-4"} ${active ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>
        ...
        <div className="mt-auto pt-6 space-y-2">
          <Link href="/settings" title={collapsed ? "Settings" : undefined} className={`flex items-center py-3.5 rounded-2xl transition-all font-semibold ${collapsed ? "justify-center px-0" : "gap-4 px-4"} ${pathname === "/settings" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>
      ...
      <main className="flex-1 flex flex-col min-w-0 pb-20 md:pb-0 relative max-w-5xl mx-auto w-full">
        ...
        <div className={`md:hidden flex items-center justify-between p-4 border-b-2 border-border bg-card z-40 ${user.isGuest ? "" : "sticky top-0"}`}>
