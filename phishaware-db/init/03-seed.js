...
const stamp = (doc, created) => Object.assign(
  { createdAt: created || now, updatedAt: now, deletedAt: null }, doc
);
...
// ---------- organizations ----------
db.organizations.insertOne(stamp({
...
db.lessons.insertMany(vectors.map((v, i) => stamp({
...
db.consents.insertMany(allUsers.map((userId) => stamp({
